...
export function Sidebar() {
	...
	return (
		<>
			{sidebarOpen && <button type="button" className="fixed inset-0 bg-black/80 z-20 lg:hidden" onClick={toggleSidebar} />}
					...
					{navItems.map((item) => {
						...
						return (
							<button
								...
								className={`btn btn-ghost w-full flex items-center justify-start gap-3 px-3 py-2 rounded-lg transition-colors ${isActive ? "bg-primary text-primary-content font-medium" : "hover:bg-base-300 text-base-content/80"}`}
							>
								<item.icon size={20} />
								{item.label}
							</button>
